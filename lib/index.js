...
async function generateTable(name, table, info, config) {
  ...
  // generate text
  let columns = '';
  for (const field in table.columns) {
    const t = table.columns[field];
    t.typeText = t2t(name, field, t, info, config);
    t.defaultValText = v2t(name, field, t, info, config);
    columns += f2t(name, field, t, info, config);
  }
  let text = flagBegin + lf + gt(name, columns, table.comment, info, config) + lf + flagEnd;
  ...
  if (!rewrite && fs.existsSync(filename)) {
    ...
    if (start >= 0) {
      ...
      if (text === rawText.substring(start, end + flagEnd.length)) {
        ...
        if (notice)notice(name, table, 'ignore');
        return 'ignore';
      }
    }
  ...
  if (notice)notice(name, table, flag);
  return flag;
}
