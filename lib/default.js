...
function gt(table, fields, comment, info, config) {
  ...
  const tableHead = `  // ${comment}
  const model = ${sequelizeText}.define('${tableName}', {
`;
  const tableTail = `  }, {
    tableName: '${tableName}',
  });`;
...
function f2t(table, field, obj, info, config) {
  ...
  const { defaultValText, foreignKey, primaryKey, uniqueKey, allowNull, typeText, isSerialKey } = obj;

  const isUnique = uniqueKey || (foreignKey && foreignKey.isUnique);

  text += spaces(3) + 'type: DataTypes.' + typeText + ',' + lf;
  text += spaces(3) + 'allowNull: ' + allowNull + ',' + lf;
  if (defaultValText !== null) {
  ...
  if (isUnique) {
    if (uniqueKey && typeof uniqueKey === 'string') {
      text += spaces(3) + "unique: '" + uniqueKey + "'," + lf;
    } else {
...
function pickQuate(type, text) {
  const last = text.length - 1;
  if (text.length < 2) return text;
  if ((text[0] !== '"' || text[last] !== '"') && (text[0] !== "'" || text[last] !== "'")) {
    return text;
  }
...
function v2t(table, field, { isSerialKey, type, defaultValue }, { dialect }, { sequelizeText }) {
  ...
  if (_.isString(defaultVal)) {
    ...
    if (_.endsWith(defaultVal, '()')) {
      ...
      val_text = '"' + pickQuate(field_type, val_text) + '"';
    }
  }
